...
async function handleFile(file) {
    // Sdílený validator: typ, velikost, prázdnost.
    ...
    showProcessing(t('analyzingAudio'), t('decodingFile'));
    ...
    try {
        ...
        const bufCheck = validateAudioBuffer(audioBuffer);
        if (!bufCheck.ok) {
            hideProcessing();
            alert(bufCheck.message);
            resetApp();
            return;
        }
        if (bufCheck.warnings && bufCheck.warnings.length > 0) {
            const proceed = confirm(bufCheck.warnings.join('\n\n') + '\n\n' + t('warnContinueAnyway'));
            if (!proceed) {
                hideProcessing();
                resetApp();
                return;
...
async function processAudio() {
    ...
    // Centralizovaná validace stretch ratio (0.5×–2×).
    // Obsahuje i kontrolu targetDuration > 0 a minimální délky.
    const ratioCheck = validateStretchRatio(originalDuration, targetDuration);
    if (!ratioCheck.ok) {
        alert(ratioCheck.message);
        return;
    }
    const tempo = ratioCheck.tempo;
